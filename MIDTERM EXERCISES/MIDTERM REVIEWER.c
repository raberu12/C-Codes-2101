 ...
 *--------------------------------------------------------------------------------*/
	...
	VHeap A;
	initVHeap(&A);
	cursorSet setA = initCursorSet(&A);
 ...
 *--------------------------------------------------------------------------------*/	
	...
	openDic D = convertToOpenDict(&setA);
 	displayOpenDict(D);
...
void insertSorted(cursorSet *A, product P)
{
	if(A->count < VH_SIZE){
		...
		for(i = A->elemIndex; i != -1 && strcmp(P.prodID, A->VHptr->VH_node[i].elem.prodID) != 0; i = A->VHptr->VH_node[i].next){}
		
		if(i == -1){
			int ndx = mallocInVHeap(A->VHptr);
			int *x;
			
			for(x = &(A->elemIndex); *x != -1 && strcmp(P.prodID, A->VHptr->VH_node[*x].elem.prodID) > 0; x = &(A->VHptr->VH_node[*x].next)){}
			
			A->VHptr->VH_node[ndx].elem = P;
			A->VHptr->VH_node[ndx].next = *x;
			*x = ndx;
			A->count++;
...
//
//        while (*ptrav != -1) {
//            if (strcmp(T.VH_node[trav].elem.prodID, dic.dicVHptr->VH_node[*ptrav].elem.prodID) == 0) {
//                alreadyInserted = 1;
//                break;
//            }
...
//                dic.dicVHptr->VH_node[node].next = dic.header[hash];
//                dic.header[hash] = node;
//            }
//        }
//
...
void displayOpenDict(openDic D)
{
	...
	for(i = 0; i < OPEN_DSIZE; i++){
		...
		for(j = D.header[i]; j != -1; j = D.dicVHptr->VH_node[j].next){
			printf("%5s", D.dicVHptr->VH_node[j].elem.prodID);
		}
	}
